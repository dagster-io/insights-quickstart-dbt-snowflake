...
def async_observation_metrics(context):
    query_id_mappings = {}
    partition_date_str = context.asset_partition_key_for_output()

    start_date = datetime.datetime.strptime(partition_date_str, "%Y-%m-%d-%H:%M").replace(
        tzinfo=datetime.timezone.utc
    )
    end_date = start_date + datetime.timedelta(hours=1, minutes=5)
